...
@implementation WKWebView (ZWWKWebViewExtension)
...
- (void)setTitleTips:(NSString *)titleTips{
    NSString *text = [titleTips stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSString *jsString = [NSString stringWithFormat:@"RE.setTitleTips(\'%@\');",text];
    [self evaluateJavaScriptRelust:jsString Callback:nil];
}
        ...
        if (hiddenPlaceHolder == FALSE) {
            [self contentHtmlTextHandleCallback:^(id  _Nullable obj, NSError * _Nullable error) {
                if (error == nil) {
                    ...
                    BOOL hiddenPlaceHolder;
                    if ([contentHtmlString containsString:@"<hr>"] || [contentHtmlString containsString:@"blockquote"]) {
                        hiddenPlaceHolder = YES;
                    }else{
                        hiddenPlaceHolder = [self getImgTags:contentHtmlString].count > 0;
                    }
                    [self placeHolderWithHidden:hiddenPlaceHolder];
                }
            }];
...
- (void)setPadding:(UIEdgeInsets)edge{
    NSString *trigger = [NSString stringWithFormat:@"RE.setPadding(\'%f\', \'%f\',\'%f\', \'%f\');",edge.top,edge.left,edge.bottom,edge.right];
    [self evaluateJavaScriptRelust:trigger Callback:nil];
}

- (void)setClickImage:(void (^)(id _Nullable obj, NSError * _Nullable error))callback{
    NSString *trigger = [NSString stringWithFormat:@"RE.setClickImage();"];
    [self evaluateJavaScriptRelust:trigger Callback:nil];
}
...
- (void)heading5 {
    NSString *trigger = @"RE.setHeading('5');";
    [self evaluateJavaScriptRelust:trigger Callback:nil];
}
- (void)normalFontSize{
...
- (void)insertLinkUrl:(NSString *)url content:(NSString*)content{
    NSString *trigger= [NSString
                        stringWithFormat:@"RE.insertLink(\'%@\', \'%@\',);",url,content];
    [self evaluateJavaScriptRelust:trigger Callback:nil];
}
...
- (void)contentBecomeFirstResponder{
    [self evaluateJavaScriptRelust:@"document.getElementById(\"article_content\").focus()" Callback:^(id  _Nullable obj, NSError * _Nullable error) {
        NSLog(@"%@",error);
    }];
}
- (void)hiddenKeyboard{
...
- (void)setContentHeight:(float)contentHeight {
    NSString *trigger = [NSString stringWithFormat:@"RE.contentHeight = %f;", contentHeight];
    [self evaluateJavaScriptRelust:trigger Callback:nil];
}

- (void)getContentOffsetHeight:(void (^)(id _Nullable, NSError * _Nullable))callback{
    NSString *trigger = [NSString stringWithFormat:@"document.body.offsetHeight;"];
    [self evaluateJavaScriptRelust:trigger Callback:callback];
}

- (void)getImageArray:(void (^)(id _Nullable, NSError * _Nullable))callback{
    NSString *trigger = [NSString stringWithFormat:@"getImageArray();"];
    [self evaluateJavaScriptRelust:trigger Callback:callback];
}
